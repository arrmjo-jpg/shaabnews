...
bench("Original OR EXISTS (orWhereHas)", function () use ($now, $locale, $categoryId) {
    $row = DB::selectOne("
        SELECT count(*) AS cnt
        FROM   articles
...
bench("Optimized OR EXISTS (skip categories join)", function () use ($now, $locale, $categoryId) {
    $row = DB::selectOne("
        SELECT count(*) AS cnt
        FROM   articles
        WHERE  status = 'published'
