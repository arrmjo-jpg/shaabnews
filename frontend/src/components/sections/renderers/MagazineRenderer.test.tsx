...
// نفس عناصر الاختبار المُستخدَمة لالتقاط __fixtures__/magazine-renderer.snapshot.html — لا
// تُغيَّر هذه المصفوفة بلا تحديث الـsnapshot معها (راجع كيفية التقاطه في سجلّ B2.2).
...
describe('MagazineRenderer — Renderer Consolidation (B2.2)', () => {
  it('matches the captured pre-extraction snapshot byte-for-byte', () => {
    const expected = fs.readFileSync(
      path.resolve(__dirname, '__fixtures__/magazine-renderer.snapshot.html'),
      'utf-8',
    );
    const actual = renderToStaticMarkup(<MagazineRenderer items={items} />);
    expect(actual).toBe(expected);
  });

  it('renders only the first 9 items even when given more', () => {
    const extra: FeedItem[] = [
      ...items,
      { ...items[0], id: 999, title: 'زائد عن الحدّ', href: '/news/999' },
    ];
    const html = renderToStaticMarkup(<MagazineRenderer items={extra} />);
    expect(html).not.toContain('زائد عن الحدّ');
  });

  it('renders nothing extra for an empty list (no crash, empty <ul>)', () => {
    const html = renderToStaticMarkup(<MagazineRenderer items={[]} />);
